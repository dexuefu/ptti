package cn.ptti.portal.web.action.upgrade;


import cn.ptti.portal.bean.upgrade.UpgradeLog;
import cn.ptti.portal.bean.upgrade.UpgradePackage;
			...
			if(upgradeId != null && !"".equals(upgradeId.trim())){
				...
				if(upgradeSystem != null){
					...
					if(upgradeSystem.getRunningStatus() <20){
						...
						//源目录路径
						String resDirPath = temp_path+upgradeSystem.getUpdatePackageFirstDirectory()+File.separator+"cn.ptti.portal"+File.separator;
						
						//当前系统使用目录
						String current_dir = StringUtils.substringAfterLast(PathUtil.path(), File.separator);
						
						if(!"cn.ptti.portal".equals(current_dir)){
							//重命名文件夹名称,和使用的目录名称一致
