...
import cn.ptti.portal.service.user.UserCustomService;
...
public class UserFormManageAction {
	...
	public String register(ModelMap model,User formbean,
		...
		if(error.size() == 0){
			...
			if(error.size() == 0){
				...
				//将访问令牌添加到Cookie
				WebUtil.addCookie(response, "cn.ptti.portal_accessToken", accessToken, 0);
				//将刷新令牌添加到Cookie
				WebUtil.addCookie(response, "cn.ptti.portal_refreshToken", refreshToken, 0);
	...
	public String loginUI(ModelMap model,String jumpUrl,
		...
		if(systemSetting.getLogin_submitQuantity() <=0){//每分钟连续登录密码错误N次时出现验证码
			...
			String userName = WebUtil.getCookieByName(request.getCookies(),"cn.ptti.portal_userName");
			if(userName != null && !"".equals(userName.trim())){
	...
	public String login2(ModelMap model,String userName, String password,Boolean rememberMe,String jumpUrl,
		...
		if(error.size() == 0){
			...
			if(user != null){
				...
				if(user.getState() >1 ){
					...
					//将访问令牌添加到Cookie
					WebUtil.addCookie(response, "cn.ptti.portal_accessToken", accessToken, maxAge);
					//将刷新令牌添加到Cookie
					WebUtil.addCookie(response, "cn.ptti.portal_refreshToken", refreshToken, maxAge);
		...
		if(error.size() >0){
			...
			//添加用户名到Cookie
			WebUtil.addCookie(response, "cn.ptti.portal_userName", userName, 60);
		}else{
			//删除每分钟原来提交次数
			settingManage.deleteSubmitQuantity("login", userName);
			WebUtil.deleteCookie(response, "cn.ptti.portal_userName");
		}
	...
	public String logout(ModelMap model,String token,
		...
		if(error.size() ==0){
			...
			WebUtil.deleteCookie(response, "cn.ptti.portal_refreshToken");
			WebUtil.deleteCookie(response, "cn.ptti.portal_accessToken");
		}
