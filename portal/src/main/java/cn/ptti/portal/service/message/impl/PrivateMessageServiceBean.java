package cn.ptti.portal.service.message.impl;

import cn.ptti.portal.bean.QueryResult;
import cn.ptti.portal.bean.message.PrivateMessage;
...
import cn.ptti.portal.utils.VersionCompare;
import cn.ptti.portal.web.action.data.MySqlDataManage;
import cn.ptti.portal.web.action.message.PrivateMessageConfig;
...
public class PrivateMessageServiceBean extends DaoSupport<PrivateMessage> implements PrivateMessageService{
	...
	public QueryResult<PrivateMessage> findPrivateMessageChatByUserId(Long userId,Long friendUserId,Integer status,int firstIndex, int maxResult){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cn.ptti.portal.bean.message.PrivateMessage_"+tableNumber);
