package cn.ptti.portal.service.user.impl;
...
import cn.ptti.portal.service.message.PrivateMessageService;
...
import cn.ptti.portal.web.action.user.PointLogConfig;
import cn.ptti.portal.web.action.user.UserDynamicConfig;
import cn.ptti.portal.web.action.user.UserLoginLogConfig;
...
public class UserServiceBean extends DaoSupport<User> implements UserService {
	...
	public PointLog findPointLogById(String pointLogId){
		...
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cn.ptti.portal.bean.user.PointLog_"+tableNumber);
	...
	public UserLoginLog findUserLoginLogById(String userLoginLogId){
		...
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cn.ptti.portal.bean.user.UserLoginLog_"+tableNumber);
				Object object  = c.newInstance();
...
/**----------------------------------- 用户动态 -------------------------------------**/
	...
	public QueryResult<UserDynamic> findUserDynamicPage(Long userId,String userName,int firstIndex, int maxResult){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cn.ptti.portal.bean.user.UserDynamic_"+tableNumber);
				Object object  = c.newInstance();
