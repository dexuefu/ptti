package cn.ptti.portal.service.follow.impl;
...
import cn.ptti.portal.web.action.follow.FollowConfig;
import cn.ptti.portal.web.action.follow.FollowerConfig;
...
public class FollowServiceBean extends DaoSupport<Follow> implements FollowService{
	...
	public Follow findById(String followId){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cn.ptti.portal.bean.follow.Follow_"+tableNumber);
	...
	public QueryResult<Follower> findFollowerByUserName(Long userId,String userName,int firstIndex, int maxResult){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cn.ptti.portal.bean.follow.Follower_"+tableNumber);
				Object object  = c.newInstance();
